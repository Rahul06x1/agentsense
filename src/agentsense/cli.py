...
from agentsense.proxy.stdio import run_stdio_proxy
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="agentsense")
    sub = parser.add_subparsers(dest="command", required=True)

    p = sub.add_parser("proxy", help="Transparent MCP proxy that traces all traffic")
    p.add_argument("--db", default="agentsense.db", help="SQLite trace store path")
    ...
    u.add_argument("--db", default="agentsense.db", help="SQLite trace store path")
    ...
    if args.command == "proxy":
        ...
        logging.getLogger("agentsense").info("trace_id=%s db=%s", trace_id, args.db)
        try:
    ...
    if args.command == "ui":
        try:
            ...
            from agentsense.ui.app import create_app
        ...
        print(f"agentsense ui → {url}  (db={args.db})", file=sys.stderr)
